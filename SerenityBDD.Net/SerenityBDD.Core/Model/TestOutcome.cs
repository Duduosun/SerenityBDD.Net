...
namespace SerenityBDD.Core.Model
{
    public class TestOutcome
    {
        ...
        private readonly TestResult IGNORED;
        private readonly TestResult SKIPPED;
        private readonly TestResult PENDING;
        ...
        private TestOutcome()
        {
            ...
            qualifier = (Optional<string>)Optional.absent();
            this.context = null;
        ...
        public TestOutcome(string name, TestCase testCase, EnvironmentVariables environmentVariables)
        {
            ...
            this.linkGenerator = Injectors.getInjector().getInstance<LinkGenerator>();
            this.flagProvider = Injectors.getInjector().getInstance<FlagProvider>();
            this.qualifier = (Optional<string>)Optional.absent();
        ...
        private TestOutcome(string methodName, TestCase testCase1, Story story, EnvironmentVariables environmentVariables1)
        {
        ...
        {
            return new PackageBasedLeafRequirements(this.environmentVariables);
        }
        ...
        //}
            ...
            public string TestFailureMessage {  get { return this.testFailureMessage; } }
        ...
        public class TestOutcomeWithEnvironmentBuilder
        {
            ...
            public Object forTest(string methodName, TestCase testCase)
            {
                return new TestOutcome(methodName, testCase, environmentVariables);
            }

            public Object forTest(string methodName, TestCase testCase, Story story)
            {
        ...
        private object withNoArguments(string s)
        {
            throw new NotImplementedException();
        }
        ...
        public IEnumerable<TestStep> getFlattenedTestSteps()
        {
            var flattenedTestSteps = new List<TestStep>();
            foreach (var step in getTestSteps())
            {
                flattenedTestSteps.Add(step);
                if (step.isAGroup())
                {
                    flattenedTestSteps.AddRange(step.getFlattenedSteps());
                }
            }
        ...
        public TestResult getResult()
        {
            ...
            TestResult className = testResultFromFailureClassname();

            var overallResults = new List<TestResult>(getCurrentTestResults());
            overallResults.Add(className);
        ...
        {
            foreach (var step in getFlattenedTestSteps())
            {
            ...
            return (Optional<TestStep>) Optional.absent();
        }

        private bool isNotBlank(string src)
        {
        ...
        {
            var allIssues = new List<string>(this.issues);
            if (thereAre(additionalIssues))
            {
                allIssues.AddRange(additionalIssues);
            }
            return ImmutableList.copyOf(allIssues);
        }

        private bool thereAre<T>(IEnumerable<T> items)
        {
            return items.Any();
        }
        ...
        {
            var allVersions = new List<string>(versions);
            if (thereAre(additionalVersions))
            {
                allVersions.AddRange(additionalVersions);
            }
        ...
        private TestCase findTestCaseFromName(string testCaseName)
        {
            try
            {
                return (testCaseName != null) ? (TestCase)Class.forName(testCaseName) : null;
            }
        ...
        private ISet<TestTag> getTagsUsingTagProviders(List<TagProvider> tagProviders)
        {
            var tags = new HashSet<TestTag>();

            foreach (var tagProvider in tagProviders)
            {
                try
                {
                    tags.AddAll(tagProvider.getTagsFor(this));
                }
                catch (theTagProviderFailedButThereIsntMuchWeCanDoAboutIt e)
                {
                    LOGGER.Error("Tag provider " + tagProvider + " failure", e);
                }
            }
            tags = (HashSet<TestTag>)removeRedundantTagsFrom(tags);
            return ImmutableSet.copyOf(tags);
        }

        private ISet<TestTag> removeRedundantTagsFrom(ISet<TestTag> tags)
        {
            var optimizedTags = new HashSet<TestTag>();
        ...
        {
            ISet<TestTag> updatedTags = new HashSet<TestTag>(getTags());
            updatedTags.Add(tag);
            this.tags = ImmutableSet.copyOf(updatedTags);
        }
        ...
        private IEnumerable<TGT> convert<SRC, TGT>(IEnumerable<SRC> src, External.Converter<SRC, TGT> converter)
        {
            return converter.Convert(src);
        }

        private External.Converter<string, string> toIssueKeys()
        {
            return new CustomConverter<string, string>(issueNumber =>
            {
        ...
        public string getQualifiedMethodName()
        {
            if ((qualifier != null) && (qualifier.isPresent()))
            {
                string qualifierWithoutSpaces = qualifier.get().Replace(" ", "_");
        ...
        private string getQualifierText()
        {
            if ((qualifier != null) && (qualifier.isPresent()))
            {
                return qualifier.get().Replace(" ", "_");
            }
            return null;
        }
